...
import com.example.audify.viewmodel.UploadViewModel
...
class UploadFragment : Fragment() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.uiState.collect { state ->
                ...
                if (state.categories.isNotEmpty()) {
                    showCategoryDialog()
                } else if (state.publishError != null) {
                    Toast.makeText(requireContext(), state.publishError, Toast.LENGTH_LONG).show()
                }
            }
        }
    }
    ...
    private fun publishToSupabase() {
        ...
        viewModel.publish(
            ...
            audioUri = audioUri ?: (recordedFile?.let { Uri.fromFile(it) }),
            coverUri = coverUri,
